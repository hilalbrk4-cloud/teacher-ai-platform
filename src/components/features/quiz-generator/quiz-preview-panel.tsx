...
import { QuizAnswerKeyPanel } from "@/components/features/quiz-generator/quiz-answer-key-panel";
import { QuizQuestionCard } from "@/components/features/quiz-generator/quiz-question-card";
import { SectionTitle } from "@/components/shared/section-title";
import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import type { Dictionary } from "@/types/i18n";
import type { Quiz, QuizFormInput, QuizGenerationStatus, QuizQuestion } from "@/types/quiz-generator";
...
interface QuizPreviewPanelProps {
  ...
  onQuestionChange: (questionId: string, updater: (question: QuizQuestion) => QuizQuestion) => void;
}
...
export function QuizPreviewPanel({
  ...
  if (quiz && (status === "success" || status === "loading")) {
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-4 py-3.5 lg:px-5">
            <SectionTitle title={quiz.title} description={`${quiz.subject} · ${quiz.gradeLevel}`} />
            <Badge
              variant="outline"
              className={
                isSaved
            ...
            >
              {isSaved ? (
                <>
                  ...
                  {t.quizGenerator.result.statusSaved}
                </>
              ) : (
                t.quizGenerator.result.statusDraft
              )}
            </Badge>
          </div>
          ...
          <div className="flex flex-col gap-4 px-4 pb-4 lg:px-5 lg:pb-5">
            <div className="flex flex-wrap gap-1.5">
              <Badge variant="secondary">{preview.summaryQuestionCount(quiz.questions.length)}</Badge>
            </div>

            {isRegenerating ? (
              <LessonPlanLoadingState
                message={t.quizGenerator.loadingMessages[loadingStage]}
                stage={loadingStage}
                totalStages={t.quizGenerator.loadingMessages.length}
              />
            ) : null}
            ...
            >
              {quiz.questions.map((question, index) => (
                <QuizQuestionCard
                  ...
                  onChange={(updater) => onQuestionChange(question.id, updater)}
                />
              ))}
            </div>
          </div>
        ...
        {!isRegenerating && quiz.includeAnswerKey ? (
          <QuizAnswerKeyPanel t={t} questions={quiz.questions} isOpen={showAnswerKey} onToggle={onToggleAnswerKey} />
        ) : null}
      </div>
    );
  }
        ...
        <div className={cn("flex flex-col gap-3", status === "loading" && "pointer-events-none opacity-60")}>
          {hasStartedFilling ? (
            <div className="flex flex-wrap gap-1.5 rounded-lg bg-muted/50 p-2.5 animate-in fade-in-0 duration-200">
              ...
              <Badge variant="secondary">{preview.summaryQuestionCount(form.questionCount)}</Badge>
            </div>
          ) : (
          ...
          <p className="text-xs text-muted-foreground">{preview.questionPendingText}</p>
        </div>
      </div>
    </Card>
  );
