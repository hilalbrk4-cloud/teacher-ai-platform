...
import { QuizActionBar } from "@/components/features/quiz-generator/quiz-action-bar";
import { QuizForm } from "@/components/features/quiz-generator/quiz-form";
import { QuizPreviewPanel } from "@/components/features/quiz-generator/quiz-preview-panel";
...
export function QuizGeneratorWorkspace() {
  ...
  const hasQuiz = status === "success" || (status === "loading" && quiz !== null);
  const hero = t.quizGenerator.hero;
      ...
      <div className="grid grid-cols-1 items-start gap-5 print:block lg:grid-cols-[minmax(0,380px)_1fr] lg:gap-6">
        <Card className="gap-0 p-4 ring-foreground/8 print:hidden lg:sticky lg:top-6 lg:p-5">
          <QuizForm
            ...
            onToggleVisualType={toggleVisualType}
            onSubmit={generate}
          />
        </Card>
